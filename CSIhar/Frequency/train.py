def main():
    def main():
        ...
        # 输出配置文件和数据集文件的路径
        print(f"[INFO] 使用的配置文件路径: {os.path.abspath(config_path)}")
        # 计算csi_dataset.py的路径
        csi_dataset_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "csi_dataset.py")
        print(f"[INFO] 使用的csi_dataset.py路径: {csi_dataset_path}")
        if not os.path.exists(csi_dataset_path):
            print(f"[WARN] csi_dataset.py文件不存在于上述路径")
        ...
        # 打印数据处理配置（仅显示Haar小波）
        print(
            f"[INFO] 数据处理配置 -> Haar小波变换: {data_cfg['use_wavelet']}(level={data_cfg['wavelet_level']}), zscore归一化: {data_cfg['use_zscore']}")
        ...
        # 5. 创建数据集（使用手动实现的Haar小波）
        # 在创建数据集的部分添加use_wavelet参数传递
        train_ds = CSIDataset(
            ...
            use_wavelet=data_cfg['use_wavelet'],  # 新增参数
