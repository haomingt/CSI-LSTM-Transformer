...
def parse_lr(lr_cfg):
    ...
    支持多种写法：
      lr: 0.001
      lr: "1e-3"
      lr: [0.0005, 0.001]  # warmup_target=0.0005, main_base=0.001
      lr: [0.001]          # 列表单值等同标量
    返回: (warmup_target_lr, main_base_lr)
    """
...
def fill_defaults(cfg):
    ...
    data.setdefault('train_split', 0.75)  # 分层抽样默认比例
    ...
    data.setdefault('use_zscore', True)  # 新增：Z-score开关
...
def warmup_lr(optimizer, warmup_target_lr, step, warmup_steps):
    """线性预热：lr = warmup_target_lr * step / warmup_steps"""
    try:
...
def collect_action_user_files(all_files, classes):
    ...
    for file_path in all_files:
        # 1. 提取动作（基于文件所在文件夹，适配data_raw_1的目录结构）
...
def main():
    ...
    if data_cfg['norm_mode'] == 'global':
        print("\n[INFO] 计算全局 min/max 用于 global 归一化 ...")
        mats = []
        # 使用训练集计算全局参数，限制样本量避免过慢
    ...
    test_ds = CSIDataset(test_files, class_to_idx, converter,
                         max_time_len=data_cfg['max_time_len'],
                         subcarriers=data_cfg['subcarriers'],
                         augment=False,
                         cache=data_cfg['cache_in_memory'],
    ...
    # 添加打印配置信息
    print(f"\n[数据预处理配置]")
    print(f"  小波去噪: {'启用' if data_cfg['use_wavelet'] else '禁用'}")
    if data_cfg['use_wavelet']:
        print(f"  小波参数: 分解层数={data_cfg['wavelet_level']}, 阈值模式={data_cfg['wavelet_threshold_mode']}")
    print(f"  Z-score标准化: {'启用' if data_cfg['use_zscore'] else '禁用'}")
