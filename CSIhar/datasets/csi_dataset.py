...
class CSIDataset(Dataset):
    ...
    数据处理流程（新增小波去噪开关控制）：
      1. 读取原始 (T,F)
      2. 列数校验
      3. 小波去噪（可选，由use_wavelet控制）
      4. Z-score标准化（可选，由use_zscore控制）
      5. 基于插值的长短帧对齐
      6. 伪彩色转换（含归一化）
      7. 随机增强 (可选)
      8. 返回 tensor
    ...
    def __init__(self,
        ...
        # 初始化时完成预处理并缓存
        if self.cache:
            print(f"[INFO] 开始预处理 {len(files)} 个文件（小波去噪: {use_wavelet}, Z-score: {use_zscore}）...")
            for i, f in enumerate(files):
                ...
                try:
                    ...
                    # 3. 小波去噪（根据开关控制是否执行）
                    if self.use_wavelet:
                        csi_processed = wavelet_denoise_csi(
                            csi,
                            level=self.wavelet_level,
                            threshold_mode=self.wavelet_threshold_mode
                        )
                    else:
                        csi_processed = csi  # 不进行小波去噪


                    # 4. Z-score标准化
                    if self.use_zscore:
                        csi_processed = zscore_normalize(csi_processed)

                    # 5. 基于插值的长短帧对齐
                    csi_aligned = interpolate_adjust_length(csi_processed, self.target_time_len)
    ...
    def __getitem__(self, idx):
        ...
        if self.cache and fpath in self._cache_store:
            ...
            # 小波
            if self.use_wavelet:
                # 用类变量标记是否已打印过，避免每个样本都打印
                if not hasattr(self, "_wavelet_printed"):
                    print(f"[实时处理-小波去噪] 启用（层数: {self.wavelet_level}, 阈值模式: {self.wavelet_threshold_mode}）")
                    self._wavelet_printed = True  # 标记已打印
                csi_processed = wavelet_denoise_csi(
                    csi,
                    level=self.wavelet_level,
                    threshold_mode=self.wavelet_threshold_mode
                ...
                if not hasattr(self, "_wavelet_printed"):
                    print(f"[实时处理-小波去噪] 禁用（use_wavelet=False）")
                    self._wavelet_printed = True
                csi_processed = csi
