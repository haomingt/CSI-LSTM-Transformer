...
# STFT处理函数
def time_to_stft(mat: np.ndarray, nperseg=128, noverlap=64, nfft=256) -> np.ndarray:
    """
    对CSI数据进行STFT变换
    输入: (时间帧, 子载波) 形状的矩阵
    输出: (频率点, 时间点) 形状的时频图（通过子载波均值降维）
    """
    T, F = mat.shape
    stft_results = []
    ...
    for f in range(F):
        ...
        # 取幅度谱并进行对数变换（增强小信号，保持原有逻辑不变）
        stft_mag = np.abs(Zxx)
        stft_mag = np.log1p(stft_mag)
        stft_results.append(stft_mag)

    # 关键修改：对子载波维度求均值（原逻辑为np.min）
    stft_mean = np.mean(stft_results, axis=0)
    return stft_mean.astype(np.float32)
...
class CSIDataset(Dataset):
    def __init__(self,
        ...
        self.use_wavelet = use_wavelet  # 小波变换开关
        self.use_zscore = use_zscore  # 是否使用zscore标准化
        self.use_stft = use_stft  # STFT开关

        # STFT参数（从配置文件传入）
        self.stft_nperseg = stft_nperseg  # 滑动窗口大小
        self.stft_noverlap = stft_noverlap  # 窗口重叠大小
        self.stft_nfft = stft_nfft  # FFT点数

        # 预处理并缓存数据
        if self.cache:
            print(f"[INFO] 开始预处理 {len(files)} 个文件（STFT+小波去噪+zscore标准化）...")
            for i, f in enumerate(files):
                ...
                try:
                    ...
                    # 调整时间长度
                    csi_time = interpolate_adjust_length(csi_time, self.target_time_len)
                    ...
                    # 选择特征转换方式：STFT或直接使用时域数据
                    if self.use_stft:
                        csi_feature = time_to_stft(
                            csi_denoised,
                        ...
                        # 直接使用时域数据（不做FFT）
                        csi_feature = csi_denoised.T  # 转置为 (子载波, 时间帧) 便于后续处理

                    # 转换为伪彩色图像
                    img = self.converter(csi_feature)
                    # 确保内存连续性和数据类型
    ...
    def __getitem__(self, idx):
        ...
        if self.cache and fpath in self._cache_store:
            ...
            if self.use_stft:
                ...
                # 直接使用时域数据（不做FFT）
                csi_feature = csi_denoised.T  # 转置为 (子载波, 时间帧)
